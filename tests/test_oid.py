...
def test_int_tuple() -> None:
    ...
    with pytest.raises(ValueError, match="OID arcs cannot be negative"):
        to_int_tuple("1.3.6.1.4.1.-10.20")

    with pytest.raises(ValueError, match="Root arc must be"):
        to_int_tuple((30, 10))

    with pytest.raises(ValueError, match="Second arc must"):
        to_int_tuple((1, 47, 123))


def test_object_identifier() -> None:
    oid = OID("1.20.300.4000.50000")
    ...
    with pytest.raises(IndexError, match="Slices need to start at 0"):
        oid[1:2]

    with pytest.raises(IndexError, match="No step supported"):
        oid[0:2:1]

    with pytest.raises(TypeError):
        oid[cast(int, "Hello World!")]
