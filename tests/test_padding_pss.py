...
pub = TextbookRsaPublicKey(65537, int.from_bytes(bytes.fromhex(
...
def test_parse_pss_options() -> None:
    ...
    with pytest.raises(TypeError, match='conflicting hash algorithms'):
        pss.parse_pss_options(pub, def_hash, rsa.PssOptions(hash_alg=hashes.sha1()), dgst_hash_alg=hashes.sha2_256())
...
def test_pad_pss() -> None:
    ...
    with pytest.raises(NotImplementedError):
        pss.pad_pss(pub, hashes.sha1(), hashlib.sha1(msg),
